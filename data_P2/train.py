...
CONFIG = {
    "racine":       "./data_f1",   # chemin vers dataset
    "image_size":   64,            # 64x64 px 
...
def entrainer(model, optimizer, loader, config, epoch_depart=0):
    ...
    À chaque itération :
      ...
      - on lui ajoute une quantité de bruit aléatoire
      - on demande au modèle de deviner ce bruit
    ...
    for epoch in range(epoch_depart, epochs_total):
        ...
        for images, _ in loader: # _ = on ignore les labels des écuries, on n'en a pas besoin
            images = images.to(device)
 
            # Bruit aléatoire pour chaque image du batch
            t = torch.randint(0, config["T"], (images.shape[0],)).to(device)
 
            # Ajouter du bruit 
            images_bruitees, vrai_bruit = model.add_noise(images, t)
 
            # Modèle devine le bruit
...
def charger_checkpoint(chemin, config):
    """
    Si l'entraînement depuis le dernier checkpoint sauvegardé.
